...
export default {
  ...
  methods: {
    ...
    handlMusicTime () {
      // 用秒数来显示当前播放进度
      const timeDisplay = Math.floor(this.music.currentTime) // 获取实时时间
      // 分钟
    ...
    handMusicBar () {
      ...
      const x = ((this.music.currentTime / duration) * 100).toFixed(2) + '%'
      slid.style.width = x
    },
    // 处理点击进度条事件
    handClickBar (e) {
    ...
    watchMusicTime () {
      ...
      this.music.oncanplaythrough = () => {
        const time = this.music.duration
        // 分钟
...
.music__main__cover img {
  width: 100%;
  height: 100%;
}
.music__main__cover.active {
...
.music__main__timeBar .time {
  display: flex;
  justify-content: space-between;
  color: #fff;
}
.music__main__timeBar .time span {
  font-size: 19px;
  line-height: 1;
}
.music__main__timeBar .bar {
  height: 8px;
...
.music__main__timeBar .bar__slid {
  position: absolute;
  left: 0;
  top: 0;
